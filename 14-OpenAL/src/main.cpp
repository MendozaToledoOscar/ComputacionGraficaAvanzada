...
int ch;
ALboolean loop;
std::vector<bool> sourcesPlay = {true, true, true, false};

bool isPressAircraft = false;
...
bool processInput(bool continueApplication) {
	...
	if (!isPressAircraft && glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS) {
		sourcesPlay[3] = true;
		isPressAircraft = true;
	}
	else if (isPressAircraft && glfwGetKey(window, GLFW_KEY_K) == GLFW_RELEASE) {
		isPressAircraft = false;
	}
...
void applicationLoop() {
	...
	while (psi) {
		...
		source3Pos[0] = modelMatrixAircraft[3].x;
		source3Pos[1] = modelMatrixAircraft[3].y;
		source3Pos[2] = modelMatrixAircraft[3].z;
		alSourcefv(source[3], AL_POSITION, source3Pos);
