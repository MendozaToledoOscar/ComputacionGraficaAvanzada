...
//Shaders para el sistema de particulas
...
void initParticleBuffers() {
	...
	// Almacenar el espacio de los buffers
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Definiendo la textura a utilizar
	Texture textureParticleFountain("../Textures/bluewater.png");
	// Carga el mapa de bits (FIBITMAP es el tipo de dato de la libreria)
	bitmap = textureParticleFountain.loadImage();
	// Convertimos el mapa de bits en un arreglo unidimensional de tipo unsigned char
	data = textureParticleFountain.convertToData(bitmap, imageWidth,
		imageHeight);
	// Creando la textura con id 1
	...
	if (data) {
		...
		std::cout << "Failed to load texture" << std::endl;
	// Libera la memoria de la textura
	textureParticleFountain.freeImage(bitmap);

	//Inicializar los buffers del sistema de particulas
	initParticleBuffers();
}
...
void destroy() {
	...
	// Eliminar los buffer del sistema de particulas
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderTerrain.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		// Settea la matriz de vista y projection al shader del sistema de particluas
